...
void Bones::ProcessMesh(const aiMesh* mesh, IMesh& cur_mesh)
{
    ...
    for (uint32_t vertex_id = 0; vertex_id < per_vertex_bone_info.size(); ++vertex_id) {
        ...
        std::copy(per_vertex_bone_info[vertex_id].begin(), per_vertex_bone_info[vertex_id].end(),
                  back_inserter(bone_info));
    }
}
...
void Bones::UpdateAnimation(RenderDevice& device, RenderCommandList& command_list, float time_in_seconds)
{
    if (!HasAnimation()) {
        return;
    }
    ...
    if (!bones_info_srv) {
        bones_info_srv = device.CreateBuffer(BindFlag::kShaderResource | BindFlag::kCopyDest,
                                             static_cast<uint32_t>(bone_info.size() * sizeof(BoneInfo)));
    }
    if (!bone_info.empty()) {
    ...
    if (!bone_srv) {
        bone_srv = device.CreateBuffer(BindFlag::kShaderResource | BindFlag::kCopyDest,
                                       static_cast<uint32_t>(bone.size() * sizeof(glm::mat4)));
    }
    if (!bone.empty()) {
...
{
    return glm::mat3(mat.a1, mat.b1, mat.c1, mat.a2, mat.b2, mat.c2, mat.a3, mat.b3, mat.c3);
}
...
void Bones::ReadNodeHeirarchy(float animation_time, const aiNode* node, const glm::mat4& parent_transform)
{
    ...
    if (node_anim) {
        ...
        glm::mat4 rotatio_mat = glm::mat4(to_glm(rotation.GetMatrix()));
...
aiVector3D Bones::CalcInterpolatedScaling(float animation_time, const aiNodeAnim* node_anim)
{
    ...
    float DeltaTime =
        (float)(node_anim->mScalingKeys[NextScalingIndex].mTime - node_anim->mScalingKeys[ScalingIndex].mTime);
...
aiQuaternion Bones::CalcInterpolatedRotation(float animation_time, const aiNodeAnim* node_anim)
{
    ...
    float DeltaTime =
        (float)(node_anim->mRotationKeys[NextRotationIndex].mTime - node_anim->mRotationKeys[RotationIndex].mTime);
...
aiVector3D Bones::CalcInterpolatedPosition(float animation_time, const aiNodeAnim* node_anim)
{
    ...
    float DeltaTime =
        (float)(node_anim->mPositionKeys[NextPositionIndex].mTime - node_anim->mPositionKeys[PositionIndex].mTime);
