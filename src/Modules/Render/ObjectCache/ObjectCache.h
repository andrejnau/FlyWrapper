...
public:
    ...
    std::shared_ptr<BindingSet> GetBindingSet(const std::shared_ptr<BindingSetLayout>& layout,
                                              const std::vector<BindingDesc>& bindings);
    std::shared_ptr<Framebuffer> GetFramebuffer(const FramebufferDesc& desc);
    std::shared_ptr<View> GetView(const std::shared_ptr<Program>& program,
...
private:
    ...
    std::map<std::pair<std::shared_ptr<BindingSetLayout>, std::vector<BindingDesc>>, std::shared_ptr<BindingSet>>
        m_binding_set_cache;
    std::map<FramebufferDesc, std::shared_ptr<Framebuffer>> m_framebuffers;
    std::map<std::tuple<std::shared_ptr<Program>, BindKey, std::shared_ptr<Resource>, LazyViewDesc>,
             std::shared_ptr<View>>
        m_views;
};
