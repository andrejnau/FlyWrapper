...
void RenderCommandListImpl::CopyBuffer(const std::shared_ptr<Resource>& src_buffer,
...
void RenderCommandListImpl::CopyTexture(const std::shared_ptr<Resource>& src_texture,
                                        const std::shared_ptr<Resource>& dst_texture,
...
void RenderCommandListImpl::UpdateSubresource(const std::shared_ptr<Resource>& resource,
...
void RenderCommandListImpl::UpdateDefaultSubresource(const std::shared_ptr<Resource>& resource,
                                                     uint32_t subresource,
                                                     const void* data,
                                                     uint32_t row_pitch,
                                                     uint32_t depth_pitch)
...
void RenderCommandListImpl::Draw(uint32_t vertex_count,
...
void RenderCommandListImpl::DrawIndexed(uint32_t index_count,
                                        uint32_t instance_count,
...
void RenderCommandListImpl::DrawIndirect(const std::shared_ptr<Resource>& argument_buffer,
...
void RenderCommandListImpl::DrawIndexedIndirect(const std::shared_ptr<Resource>& argument_buffer,
                                                uint64_t argument_buffer_offset)
{
    Apply();
    BufferBarrier(argument_buffer, ResourceState::kIndirectArgument);
...
void RenderCommandListImpl::DrawIndirectCount(const std::shared_ptr<Resource>& argument_buffer,
                                              uint64_t argument_buffer_offset,
                                              ...
                                              uint32_t stride)
{
    Apply();
    BufferBarrier(argument_buffer, ResourceState::kIndirectArgument);
    m_command_list->DrawIndirectCount(argument_buffer, argument_buffer_offset, count_buffer, count_buffer_offset,
                                      max_draw_count, stride);
}

void RenderCommandListImpl::DrawIndexedIndirectCount(const std::shared_ptr<Resource>& argument_buffer,
                                                     uint64_t argument_buffer_offset,
                                                     const std::shared_ptr<Resource>& count_buffer,
                                                     uint64_t count_buffer_offset,
                                                     uint32_t max_draw_count,
                                                     uint32_t stride)
{
    Apply();
    BufferBarrier(argument_buffer, ResourceState::kIndirectArgument);
    m_command_list->DrawIndexedIndirectCount(argument_buffer, argument_buffer_offset, count_buffer, count_buffer_offset,
                                             max_draw_count, stride);
}

void RenderCommandListImpl::Dispatch(uint32_t thread_group_count_x,
...
void RenderCommandListImpl::DispatchIndirect(const std::shared_ptr<Resource>& argument_buffer,
                                             uint64_t argument_buffer_offset)
{
    Apply();
    BufferBarrier(argument_buffer, ResourceState::kIndirectArgument);
...
void RenderCommandListImpl::ViewBarrier(const std::shared_ptr<View>& view, ResourceState state)
{
    ...
    ImageBarrier(view->GetResource(), view->GetBaseMipLevel(), view->GetLevelCount(), view->GetBaseArrayLayer(),
                 view->GetLayerCount(), state);
}

void RenderCommandListImpl::ImageBarrier(const std::shared_ptr<Resource>& resource,
...
void RenderCommandListImpl::BuildBottomLevelAS(const std::shared_ptr<Resource>& src,
...
void RenderCommandListImpl::BuildTopLevelAS(
...
void RenderCommandListImpl::CopyAccelerationStructure(const std::shared_ptr<Resource>& src,
                                                      const std::shared_ptr<Resource>& dst,
    ...
    if (m_program->HasShader(ShaderType::kCompute)) {
        ...
        for (const auto& shader : m_program->GetShaders()) {
            for (const auto& entry_point : shader->GetReflection()->GetEntryPoints()) {
                ...
                case ShaderKind::kClosestHit:
                    groups.push_back(
                        { RayTracingShaderGroupType::kTrianglesHitGroup, 0, shader->GetId(entry_point.name) });
                    break;
                case ShaderKind::kAnyHit:
                    groups.push_back(
                        { RayTracingShaderGroupType::kTrianglesHitGroup, 0, 0, shader->GetId(entry_point.name) });
                    break;
                case ShaderKind::kIntersection:
                    groups.push_back(
                        { RayTracingShaderGroupType::kProceduralHitGroup, 0, 0, 0, shader->GetId(entry_point.name) });
                    break;
                }
            }
        }
...
{
    decltype(auto) shader_handles =
        pipeline->GetRayTracingShaderGroupHandles(0, m_ray_tracing_pipeline_desc.groups.size());
    ...
    for (const auto& shader : m_program->GetShaders()) {
        for (const auto& entry_point : shader->GetReflection()->GetEntryPoints()) {
            switch (entry_point.kind) {
            case ShaderKind::kRayGeneration:
                m_shader_table->UpdateUploadBuffer(
                    m_shader_tables.raygen.offset + raygen_offset,
                    shader_handles.data() + m_device.GetShaderGroupHandleSize() * group_id,
                    m_device.GetShaderGroupHandleSize());
                raygen_offset += m_device.GetShaderTableAlignment();
                break;
            case ShaderKind::kMiss:
                m_shader_table->UpdateUploadBuffer(
                    m_shader_tables.miss.offset + miss_offset,
                    shader_handles.data() + m_device.GetShaderGroupHandleSize() * group_id,
                    m_device.GetShaderGroupHandleSize());
            ...
            case ShaderKind::kCallable:
                m_shader_table->UpdateUploadBuffer(
                    m_shader_tables.callable.offset + callable_offset,
                    shader_handles.data() + m_device.GetShaderGroupHandleSize() * group_id,
                    m_device.GetShaderGroupHandleSize());
            ...
            case ShaderKind::kIntersection:
                m_shader_table->UpdateUploadBuffer(
                    m_shader_tables.hit.offset + hit_offset,
                    shader_handles.data() + m_device.GetShaderGroupHandleSize() * group_id,
                    m_device.GetShaderGroupHandleSize());
...
void RenderCommandListImpl::Attach(const BindKey& bind_key,
...
void RenderCommandListImpl::LazyResourceBarrier(const std::vector<LazyResourceBarrierDesc>& barriers)
{
    ...
    for (const auto& barrier : barriers) {
        ...
        if (state_tracker.HasResourceState() && barrier.base_mip_level == 0 &&
            ...
            if (manual_barrier.state_before != ResourceState::kCommon) {
            ...
            for (uint32_t i = 0; i < barrier.level_count; ++i) {
                for (uint32_t j = 0; j < barrier.layer_count; ++j) {
                    ...
                    manual_barrier.state_before = state_tracker.GetSubresourceState(manual_barrier.base_mip_level,
                                                                                    manual_barrier.base_array_layer);
                    manual_barrier.state_after = barrier.state;
                    state_tracker.SetSubresourceState(manual_barrier.base_mip_level, manual_barrier.base_array_layer,
                                                      manual_barrier.state_after);
                    if (manual_barrier.state_before != ResourceState::kCommon) {
                        manual_barriers.emplace_back(manual_barrier);
                    } else {
                        m_lazy_barriers.emplace_back(manual_barrier);
