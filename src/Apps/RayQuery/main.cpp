...
int main(int argc, char* argv[])
{
    ...
    // build BLAS
    std::shared_ptr<Resource> positions = device->CreateBuffer(BindFlag::kVertexBuffer | BindFlag::kCopyDest,
                                                               sizeof(glm::vec3) * model.meshes[0].positions.size());
    upload_command_list->UpdateSubresource(positions, 0, model.meshes[0].positions.data(), 0, 0);
    std::shared_ptr<Resource> indices = device->CreateBuffer(BindFlag::kIndexBuffer | BindFlag::kCopyDest,
                                                             sizeof(uint32_t) * model.meshes[0].indices.size());
    ...
    std::shared_ptr<Resource> bottom =
        device->CreateBottomLevelAS({ raytracing_geometry_desc }, BuildAccelerationStructureFlags::kNone);
